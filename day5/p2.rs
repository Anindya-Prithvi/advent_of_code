    ...
    let _input = fs::read_to_string("input.txt")
        ...
        .for_each(|l| {
            ...
            if start_dest[0][0] == start_dest[1][0] {
                ...
                // diagonal
                let xsign = (start_dest[1][0] - start_dest[0][0]).signum();
                let ysign = (start_dest[1][1] - start_dest[0][1]).signum();
                let travel = (start_dest[1][0] - start_dest[0][0]).abs();
                for i in 0..=travel {
                    matrix[(start_dest[0][0] + i * xsign) as usize]
                        [(start_dest[0][1] + i * ysign) as usize] += 1;
                }
            }
        });
